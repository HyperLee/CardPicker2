...
CardPicker2 是一個單機單人的本機餐點抽卡網站。使用者可以用老虎機式流程在早餐、午餐、晚餐之間抽出餐點，也可以用隨機模式從全部可抽卡牌中抽選，並透過條件篩選、近期防重複、收藏與手動排除來管理日常餐點選擇。

專案採用 ASP.NET Core Razor Pages 與單一本機 JSON 檔保存卡牌庫，適合在本機瀏覽器中管理常吃、想吃、暫時不想再抽到或需要保留歷史紀錄的餐點卡牌。

## 功能

- 老虎機式餐點抽卡流程：餐別選擇、投幣確認、拉桿啟動、轉動狀態與結果揭示。
- 正常模式與隨機模式：正常模式依餐別抽卡，隨機模式從全部可抽卡牌抽卡。
- 條件篩選抽卡：支援價格區間、準備或等待時間、飲食偏好、最高可接受辣度與標籤。
- 近期防重複：預設避免最近 3 次成功抽中的卡牌，N 可設為 0 到 10，0 等同關閉。
- 收藏與排除抽卡：收藏協助辨識與篩選；排除抽卡會讓卡牌離開未來抽卡候選池，但仍保留在卡牌庫中。
- 結果區偏好整理：成功抽出餐點後，可直接收藏或排除剛抽中的卡牌，且不會重新抽卡或修改既有歷史。
- 卡牌庫管理：瀏覽、搜尋、查看詳情、新增、編輯、刪除、收藏與取消排除餐點卡牌。
- 卡牌決策資訊：每張卡牌可維護標籤、價格、時間、飲食偏好與辣度。
- 抽卡歷史與統計：顯示總成功抽取次數、單卡抽中次數、歷史機率、卡牌狀態與輪替摘要。
...
4. 視需要調整「避免最近重複」與最近成功抽卡次數 N。
5. 勾選投幣確認並啟動抽卡。
6. 查看抽出的餐點、決策資訊摘要、輪替摘要與抽卡統計。
7. 在結果區直接收藏、取消收藏、排除或取消排除剛抽中的餐點。

卡牌庫 `/Cards` 可用關鍵字、餐別、決策條件、收藏狀態與可抽狀態搜尋。新增與編輯卡牌時，可同時維護繁體中文與英文餐點內容，以及可選的決策資訊。已排除抽卡的卡牌預設仍會顯示，方便使用者找到並取消排除。

## 抽卡規則

抽卡結果只由伺服器端候選池與亂數選擇決定，不受動畫時間、顯示順序、主題或語系影響。候選池建立順序如下：

1. 載入有效且未刪除的卡牌。
2. 移除手動標記為排除抽卡的卡牌。
3. 依正常模式或隨機模式建立基本候選池。
4. 套用價格、時間、飲食偏好、辣度與標籤等決策條件。
5. 套用近期防重複規則，排除最近 N 次成功抽卡中仍存在於本次候選池的卡牌。
6. 從最後候選池內等機率抽出一張卡牌。

若同一個成功抽卡操作被重複提交，系統會重顯原本的結果與輪替快照，不重新抽選。收藏不會改變候選池、抽中權重、歷史、統計或防重複規則。
...
├── Services/                  # JSON 持久化、抽卡、候選池、搜尋、統計、偏好與驗證
...
├── 006-card-rotation-cooldown/
└── 007-card-preference-controls/
```
...
目前程式支援 schema v5，保存卡牌內容、決策資訊、偏好狀態、成功抽卡歷史、統計所需資料與輪替快照。合法的 v1-v4 資料可載入並在下一次成功寫入時遷移；系統不會只為了格式化或強制升版而重寫 `cards.json`。

寫入採同目錄暫存檔與原子替換流程。抽卡結果、歷史紀錄、卡牌新增、編輯、刪除、偏好更新與 metadata 更新都必須完整成功或完整失敗。
...
目前功能脈絡以 `specs/007-card-preference-controls/` 為主要來源，並建立在前序功能之上：
...
- `006-card-rotation-cooldown`: 最近 N 次成功抽卡防重複與輪替快照。
- `007-card-preference-controls`: 收藏、手動排除抽卡、偏好篩選與結果區偏好操作。
