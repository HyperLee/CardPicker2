...
**Input**: `specs/004-draw-mode-statistics/` 的 `spec.md`、`plan.md`、`research.md`、`data-model.md`、`quickstart.md`、`contracts/ui-contract.md`，並保留 `specs/003-bilingual-language-toggle/` 的雙語 baseline。
**Prerequisites**: ASP.NET Core Razor Pages app、既有雙語卡牌 schema v2、xUnit/Moq/WebApplicationFactory/Playwright 測試專案。
**Tests**: 憲章與本功能 quickstart 明確要求測試優先，因此每個行為故事都先建立失敗測試，再實作。
**Organization**: 任務依使用者故事分組，讓 P1 可以作為 MVP 獨立完成與驗證，P2 與 P3 可在 foundation 完成後平行推進。
...
**Purpose**: 確認目前專案可還原與建置，並建立後續測試可重用的資料與 WebApplicationFactory 輔助工具。
...
**Purpose**: 建立 schema v3、抽卡歷史、卡牌狀態、檔案協調與 migration 基礎；所有 user story 都依賴此階段。

**Critical**: 未完成本階段前，不要開始 US1/US2/US3 的實作任務。
...
**Checkpoint**: schema v3 與檔案一致性 foundation 完成，user story implementation 可開始。

---

## Phase 3: User Story 1 - 依抽卡模式取得公平結果 (Priority: P1) MVP

**Goal**: 首頁支援正常模式與隨機模式；正常模式只從指定餐別 active cards 抽卡，隨機模式從全部 active cards 抽卡；首次成功抽卡只新增一筆歷史，同一 operation 重送 replay 原結果。

**Independent Test**: 使用具有早餐、午餐、晚餐 active cards 的測試資料，POST normal/random draw，驗證候選池、公平性標稱機率、empty/blocked/missing coin failure、idempotent replay 與 Anti-Forgery。
...
**Checkpoint**: US1 可作為 MVP 獨立展示，包含 normal/random、公平候選池與 idempotent draw history。

---

## Phase 4: User Story 2 - 查看成功抽卡次數與卡牌歷史機率 (Priority: P2)

**Goal**: 首頁顯示總成功抽卡次數與統計表；統計由成功歷史重建，失敗嘗試與 replay 不增加分母。

**Independent Test**: 使用已知 draw history document 載入首頁，驗證 total、per-card draw count、historical probability、zero-history empty state、active zero rows 與 persistence after restart。

### Tests for User Story 2

> 先撰寫這些測試，確認在實作前失敗。

- [ ] T036 [P] [US2] 新增統計投影失敗測試於 `tests/CardPicker2.UnitTests/Services/DrawStatisticsServiceTests.cs`，覆蓋 total count、per-card count、probability formula、zero-history null probability、active zero rows、deleted-with-history rows。
- [ ] T037 [P] [US2] 新增首頁統計整合失敗測試於 `tests/CardPicker2.IntegrationTests/Pages/DrawStatisticsPageTests.cs`，覆蓋總成功抽取次數、統計表欄位、無歷史空狀態、不顯示誤導性每卡 0%。
...
**Checkpoint**: US1 與 US2 可同時運作，成功 draw 會持久化 history 並立即更新首頁統計。

---

## Phase 5: User Story 3 - 保留卡牌歷史並避免誤導性呈現 (Priority: P3)

**Goal**: 有成功歷史的卡牌刪除後 retained as deleted，不再進候選池但保留統計列；改名/翻譯更新不切分統計；語系、動畫、reduced motion 與文案只影響呈現。

**Independent Test**: 讓卡牌先成功抽中，再刪除、改名、切換語系與啟用 reduced motion，驗證同一 card ID 統計延續、deleted card 不再被抽出、UI 不暗示機率改變。
...
- [ ] T054 [US3] 執行 `dotnet test CardPicker2.sln --filter "CardDeletionRetention|DeletedCardStatistics|DrawModeLocalizationInvariant|DrawCopyBoundary"`，確認 `CardPicker2.sln` 的 US3 新測試在實作前失敗。
...
- [ ] T067 [US3] 執行 `dotnet test CardPicker2.sln --filter "CardDeletionRetention|DeletedCardStatistics|DrawModeLocalizationInvariant|DrawCopyBoundary"`，確認 `CardPicker2.sln` 的 US3 測試通過。

**Checkpoint**: 所有使用者故事均可獨立驗證，且卡牌生命週期、語系與呈現層不影響抽卡公平性或統計。

---

## Phase 6: Polish & Cross-Cutting Concerns

**Purpose**: 補齊安全、日誌、公開 API 文件註解、完整驗證與手動檢查。
...
- [ ] T072 更新新增公開 API XML 文件註解於 `CardPicker2/Models/DrawMode.cs`、`CardPicker2/Models/DrawOperation.cs`、`CardPicker2/Models/DrawHistoryRecord.cs`、`CardPicker2/Models/CardDrawStatistic.cs`、`CardPicker2/Models/DrawStatisticsSummary.cs`、`CardPicker2/Services/DrawCandidatePoolBuilder.cs`、`CardPicker2/Services/DrawStatisticsService.cs`。
- [ ] T073 執行 `dotnet build CardPicker2.sln`，確認 `CardPicker2.sln` 無新增 build warning 或 formatting/naming 違規。
- [ ] T074 執行 `dotnet test CardPicker2.sln`，確認 `CardPicker2.sln` 全部單元、整合、browser/security tests 通過。
- [ ] T075 依 `specs/004-draw-mode-statistics/quickstart.md` 完成手動或 browser automation 驗證，覆蓋 normal mode、random mode、重複提交、統計表、刪除後歷史保留、改名/翻譯更新、缺檔/corrupted 隔離資料、reduced motion、RWD、安全與觀察性。
...
- **Phase 2 Foundational**: 依賴 Phase 1，且封鎖所有 user story。
- **Phase 3 US1**: 依賴 Phase 2，是 MVP。
- **Phase 4 US2**: 依賴 Phase 2；可在 US1 服務契約穩定後與 US3 部分平行，但最終需要與 US1 draw history 寫入整合。
- **Phase 5 US3**: 依賴 Phase 2；刪除後統計呈現需要 US2 統計模型，候選池排除 deleted 需要 US1 candidate pool。
- **Phase 6 Polish**: 依賴欲交付的 user stories 完成。
...
Phase 1 Setup
  -> Phase 2 Foundational
      -> US1 Draw modes and idempotency
      -> US2 Statistics projection and homepage table
      -> US3 Deleted retention and presentation invariants
          -> Phase 6 Polish
```
...
- 每個 checkpoint 後執行該 story 的 filter tests，再進入下一優先級。
...
- T005 與 T006 可平行撰寫 foundation failing tests。
...
- T068、T069、T071 可平行補齊 cross-cutting tests。
...
```bash
# 平行撰寫 US1 failing tests:
Task: "T019 [US1] tests/CardPicker2.UnitTests/Models/DrawModeTests.cs"
Task: "T020 [US1] tests/CardPicker2.UnitTests/Services/DrawCandidatePoolBuilderTests.cs"
Task: "T021 [US1] tests/CardPicker2.UnitTests/Services/CardLibraryDrawModeTests.cs"
Task: "T022 [US1] tests/CardPicker2.UnitTests/Services/DrawIdempotencyTests.cs"
Task: "T023 [US1] tests/CardPicker2.IntegrationTests/Pages/DrawModePageTests.cs"

# UI copy and assets after contracts stabilize:
Task: "T031 [US1] CardPicker2/Resources/SharedResource.zh-TW.resx"
Task: "T032 [US1] CardPicker2/Resources/SharedResource.en-US.resx"
Task: "T033 [US1] CardPicker2/wwwroot/js/site.js"
Task: "T034 [US1] CardPicker2/wwwroot/css/site.css"
```

## Parallel Example: User Story 2

```bash
# 平行撰寫 US2 failing tests:
Task: "T036 [US2] tests/CardPicker2.UnitTests/Services/DrawStatisticsServiceTests.cs"
Task: "T037 [US2] tests/CardPicker2.IntegrationTests/Pages/DrawStatisticsPageTests.cs"
Task: "T038 [US2] tests/CardPicker2.IntegrationTests/Pages/DrawHistoryPersistenceTests.cs"

# 統計 UI copy and responsive work:
Task: "T045 [US2] CardPicker2/Resources/SharedResource.zh-TW.resx"
Task: "T046 [US2] CardPicker2/Resources/SharedResource.en-US.resx"
Task: "T047 [US2] CardPicker2/wwwroot/css/site.css"
```

## Parallel Example: User Story 3

```bash
# 平行撰寫 US3 failing tests:
Task: "T049 [US3] tests/CardPicker2.UnitTests/Services/CardDeletionRetentionTests.cs"
Task: "T050 [US3] tests/CardPicker2.IntegrationTests/Pages/DeletedCardStatisticsPageTests.cs"
Task: "T051 [US3] tests/CardPicker2.IntegrationTests/Pages/DrawModeLocalizationInvariantTests.cs"
Task: "T052 [US3] tests/CardPicker2.IntegrationTests/Browser/DrawModeResponsiveAccessibilityTests.cs"
Task: "T053 [US3] tests/CardPicker2.UnitTests/Services/DrawCopyBoundaryTests.cs"

# 呈現層可平行處理:
Task: "T063 [US3] CardPicker2/Resources/SharedResource.zh-TW.resx"
Task: "T064 [US3] CardPicker2/Resources/SharedResource.en-US.resx"
Task: "T065 [US3] CardPicker2/wwwroot/css/site.css"
Task: "T066 [US3] CardPicker2/wwwroot/js/site.js"
```
...
1. Setup + Foundational: schema v3、history、status、file coordinator。
...
5. Polish: security/logging/XML docs/full test/manual verification。
...
- production HSTS/CSP、Anti-Forgery、結構化日誌與敏感資訊禁止輸出皆有測試證據。
...
MVP 範圍為 Phase 1、Phase 2、Phase 3，也就是 US1「依抽卡模式取得公平結果」。完成後即可交付 normal/random 抽卡、schema v3 history append 與 idempotent replay；US2/US3 可作為後續增量。
